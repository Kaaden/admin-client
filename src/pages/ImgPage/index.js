...
class Page extends Component {
    ...
    onDel = async (id) => {
        const { dispatch } = this.props
        const { data } = await axios.post("http://kaaden.orrzt.com/api/imgChange", qs.stringify({ id, type: "del" }))
        if (data.isok) {
            dispatch({ type: "admin/findImg" })
        }
    }
...
class UpLoadImg extends Component {
    ...
    handleUp = async () => {
        ...
        const { data } = await axios.post("http://kaaden.orrzt.com/api/imgChange", qs.stringify({ id, img }))
